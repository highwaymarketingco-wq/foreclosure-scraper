...
| `counties_sc.greenville_mie_adverts` | `https://mie.greenvillejournal.com` — Greenville SC Master-in-Equity foreclosure adverts. 772 cases with TMS, TOTAL JUDGMENT DEBT, street address, sale date, plaintiff/defendant. Overturns the "SC per-case DETAIL is ABSENT" row in blocked_sources_forensic.md; publicindex.sccourts.org is NOT touched. Sitemap returns HTTP 404 with valid XML — ignore the status. |
