...
|---|---|---|---|---|---|
| 1 | **SC coastal county-native parcels** (Charleston `gisccapps`, Beaufort `gis.beaufortcountysc.gov`, Georgetown `gis1.georgetowncountysc.org`, Colleton Hub) | resolver + S12 (+ Beaufort S11 exemption) | open ArcGIS, owner+mailing+sale | **new SC_GIS map in `enrichment_arcgis.py`** (SC currently routes through SCDOT `SC_Parcels`) | Med — resolver is a critical/bug-prone path; scope deliberately + test |
