...
| Cleveland NC | 1,169 | 51% | 52% | 54% | 39% | 3% | debt |
| Henderson NC | 2,773 | 63% | 75% | 64% > **80%** | 42% | 52% | phone |
| Polk NC | 470 | 56% | 56% | 59% | 36% | 9% | debt |
| Gaston NC | 9,810 | 81% | 67% | 14% > **86%** | 12% | 1% | debt, phone |
| Buncombe NC | 8,949 | 71% | 70% | 70% > **76%** | 65% | 23% > 25% | debt |
| Transylvania NC | 6,204 | 71% | 92% | 5% > **92%** | 3% | 1% > 7% | phone, debt |
| McDowell NC | 2,147 | 68% | 82% | 79% > **89%** | 44% | 67% | phone |
| Lincoln NC | 3,632 | 73% | 81% | 27% > **87%** | 21% | 13% | debt |
| Mitchell NC | 238 | 60% | 60% | 66% | 29% | 2% | debt |
| Burke NC | 1,391 | 59% | **31%** | 42% | 32% | 4% | value |
| Spartanburg SC | 15,311 | 53% > 59% | 42% > 47% | 52% > 59% | 5% | 13% > **38%** | phone |
| Anderson SC | 2,620 | **15%** | **16%** | **11%** | 9% | 24% | identity, mail |
| Pickens SC | 4,629 | 47% > 52% | 46% > **5%** (correction) | 58% > 69% | 8% | 43% | value |
| Oconee SC | 3,309 | 50% | 52% | 68% | 5% | 33% > **74%** | phone |
| Cherokee SC | 2,949 | 37% | 38% | **8%** | 3% | 6% > **43%** | mail |
...
11 of 180 county-by-family cells are empty. Full grid: run the ledger script.
...
| incarceration in Cleveland, Transylvania, Lincoln | Buildable now: standalone roster backfill (J-02) plus the re-query fix (J-01). Rosters verified live in July and August. |
| incarceration in Oconee, Union, Mitchell | State-prison name match only. No county roster is known or reachable. |
...
Tiers per lead after all of it: HOT 1,971, WARM 72,386, COLD 96,168. A mailable owner gates HOT,
which is why HOT rose as mail coverage rose.

## What is left to build (measured, ranked)

| Item | Counties | Lift | Status |
|---|---|---|---|
| Anderson offline owner roll into the name resolver (I-01) | Anderson | identity, mail, value from 15/11/16% | Build-ready. The live layer no longer publishes owners, so the backed-up roll is the only source. |
| Finish the per-lead mailing pass (Spartanburg 6,623 leads, Burke, Cleveland, Polk, Mitchell) | 6 | mail | Build-ready. About 15 leads a minute, polite by design; needs a multi-hour run. |
| Jail-roster backfill and the incarceration re-query fix (J-02, J-01) | 6 | 3 empty cells filled | Build-ready. |
| Cherokee situs from the ledger row (I-04) | Cherokee | identity | Build-ready, small. |
| Burke value (31%) | Burke | value | Needs recon. Not yet scoped. |
| Deed-index scrapers for `repeat_tax_loss` | 7 buildable, 11 walled | about 790 loss deeds a year footprint-wide | Scoping notes are kept local for now. Small signal even fully built. |
