...
Cumberland County publishes tax foreclosure properties on its county
website. Properties with delinquent taxes that are being processed for
foreclosure auction.
...
PAGE_URL = "https://www.co.cumberland.nc.us/departments/tax/tax-administration/tax-foreclosures"
...
class CumberlandTaxForeclosure(BaseScraper):
    ...
    async def fetch(self) -> Iterable[Listing]:
        ...
        for row in rows:
            ...
            # Extract amount owed if present
            amount = None
            for c in clean:
                m = re.search(r"\$[\d,]+\.?\d*", c)
                if m:
            ...
            out.append(Listing(
                source="counties_nc.cumberland_tax_foreclosure",
                source_url=PAGE_URL,
                ...
                raw={"cumberland_tax_foreclosure": {"cells": clean[:10]}},
            ))
