"""Fairfield County SC - Delinquent Tax Sale properties.

Fairfield County publishes delinquent tax sale properties on its county
...
PAGE_URL = "https://www.fairfieldsc.com/departments/treasurer"
...
class FairfieldDelinquentTax(BaseScraper):
    ...
    async def fetch(self) -> Iterable[Listing]:
        ...
        try:
            ...
            log.warning("fairfield_tax.fetch_fail", error=str(exc)[:160])
            return out
        ...
        for row in rows:
            ...
            out.append(Listing(
                ...
                raw={"fairfield_delinquent_tax": {"cells": clean[:10]}},
            ))

        log.info("fairfield_tax.done", count=len(out))
        return out
