...
# Situs embedded in the result/notes cell, e.g.
# "Completed-05/29/2026 Sale # 1 161 Catnip Trail, Landrum". \b word
# boundaries on the street-type token avoid matching inside other words.
_GREENVILLE_ADDR_RE = re.compile(
...
class GreenvilleMasterInEquity(BaseScraper):
    ...
    def _parse_roster(self, html: str, roster_url: str) -> list[Listing]:
        ...
        for tr in tree.css("tr.standardRow, tr.altRow"):
            ...
            # A real Greenville TMS starts with a digit and is dot/dash
            # separated (e.g. "0646.0501008.00", "063115-01-06300"). On some
            # rows the column layout drifts and an attorney name lands here —
            # reject anything that isn't TMS-shaped so we don't store a name
            # as a parcel id.
            tms = tms_raw if re.match(r"^\d[\d.\-\s]{4,}$", tms_raw) else ""
            ...
            # The notes/result cell often embeds the real situs after the
            # sale number, e.g. "Completed-05/29/2026 Sale # 1 161 Catnip
            # Trail, Landrum". Pull it so the listing has a real address
            # instead of relying solely on TMS->GIS resolution.
            street_address = None
            addr_m = _GREENVILLE_ADDR_RE.search(notes)
            if addr_m:
