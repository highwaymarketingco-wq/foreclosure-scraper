...
async def main() -> int:
    ...
    # is silently reverted, with no error anywhere. See web_artifact.board_lock.
    try:
...
async def _run() -> int:
    ...
    # a shard set describing the PREVIOUS one: phones show stale owner/mailing
    # data, and because a shard is joined to the board BY ARRAY INDEX, any
    # change to the record count or order hands one lead's comps, vision and
    # CAMA to a different lead's address. Desktop looks perfect throughout.
    #
    # WHAT USED TO BE HERE was `if (docs/listings_slim.json.gz).exists() or
    # (docs/detail_shards).exists(): return 0`, followed by a git
    # add/commit/push block gated on OM_PUBLISH. The comment described that as a
    # conditional guard. It has not been conditional since both payloads started
    # shipping on every publish — both always exist, the branch always fires,
    # and the publish block below it was dead code that nothing had reached in
    # months. A guard whose comment claims it sometimes lets you through is
    # worse than no comment: it sends the next reader looking for the run where
    # it did.
    #
    ...
    # from one payload. The backfill ships on the next publish, joined correctly.
    print(f"[{time.strftime('%H:%M:%S')}] docs/listings.json updated in place "
          f"({mailable} mailable). NOT PUBLISHING — this script cannot "
