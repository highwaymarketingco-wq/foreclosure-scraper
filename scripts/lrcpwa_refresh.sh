...
if pgrep -f "merge_today_sources.py" >/dev/null || pgrep -f "foreclosure_scraper.__main__\|-m foreclosure_scraper\|run_local.sh" >/dev/null; then
...
git add docs/listings.json docs/run_meta.json docs/parcel_photos 2>/dev/null
